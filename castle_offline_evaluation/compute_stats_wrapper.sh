...
JOB_NAME="compute_stats_112_mask_net_prediction_thresholds_spars0.001_minus4k_ref2_val"
HPC="jsc" # jsc, dkrz

CONFIG="${TRAINING_DIR}/cfg_mask_net.yml"

PLOT_DIR="${TRAINING_DIR}/plots_offline_evaluation/stats"
mkdir -p "$PLOT_DIR"
...
display_help() {
  echo ""
  echo "Wrapper for SLURM batch script for computing stats."
  echo ""
  echo "Usage: ./compute_stats_wrapper.sh -h"
  echo ""
  echo " Options:"
...
################
if [[ $HPC == "jsc" ]]; then
  BATCH_SCRIPT="compute_stats_sbatch_jsc.sh"
elif [[ $HPC == "dkrz" ]]; then
  BATCH_SCRIPT="compute_stats_sbatch_dkrz.sh"
else
  echo -e "\nUnknown HPC ${HPC}."
  error_exit
fi
