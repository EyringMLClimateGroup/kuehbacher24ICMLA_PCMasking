...
display_help() {
  echo ""
  echo "SLURM batch script for plotting lat-lon plots."
  echo ""
  echo "Usage: sbatch -J job_name --output slurm_output_logs --error slurm_error_logs plot_double_xy_sbatch_jsc.sh -c config.yml -p plot_dir -l log_dir [-j job_name]"
  echo ""
  echo " Options:"
...
conda run --cwd "$PROJECT_ROOT" --no-capture-output -n kuehbacher1_py3.9_tf python -u -m castle_offline_evaluation.main_castle_plot_double_xy -c "$CONFIG" -p "$PLOT_DIR" >"${PYTHON_DIR}/${JOB_NAME}_python_${SLURM_JOB_ID}.out"
