...
base_dir="output_castle/training_65_castle_simplified_spars0.001_matrix_norm"
HPC="jsc" # jsc, dkrz

job_name_base="training_65_castle_simplified_spars0.001_matrix_norm"
...
display_help() {
  ...
  echo ""
  echo "Usage: $0 [-h] [-i inputs_list.txt] [-o outputs_list.txt] [-m outputs_map.txt] [-n nodes] [-c config.yml] [-l load_ckp_weight] [-t continue_training] [-f fine_tune_config] [-p HPC] [-s seed]"
  echo ""
  echo " Options:"
...
if [ $# -eq 0 ]; then
  ...
  # Parse options
  while getopts "i:o:m:n:c:l:t:f:p:s:h" opt; do
    case ${opt} in
    h)
      display_help
      exit 0
