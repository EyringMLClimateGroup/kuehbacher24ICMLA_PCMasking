...
def test_create_model_description_gumbel_softmax_model(setup_str, strategy, request):
...
def test_create_model_description_gumbel_softmax_model_distributed_value_error(mocked_visible_devices, setup_str, request):
    setup = request.getfixturevalue(setup_str)
...
@pytest.mark.parametrize("strategy", ["", "mirrored"])
@pytest.mark.parametrize("setup_str", ["setup_gumbel_softmax_single_output_model_2d"])
def test_train_and_save_model_description_gumbel_softmax_model(setup_str, strategy, request):
    setup = request.getfixturevalue(setup_str)
    setup = set_strategy(setup, strategy)
...
@pytest.mark.parametrize("strategy", ["", "mirrored"])
@pytest.mark.parametrize("setup_str", ["setup_gumbel_softmax_single_output_model_2d"])
def test_load_model_description_gumbel_softmax_model(setup_str, strategy, request):
    setup = request.getfixturevalue(setup_str)
