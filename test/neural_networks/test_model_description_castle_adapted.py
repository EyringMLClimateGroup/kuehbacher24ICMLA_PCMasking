...
def test_create_model_description_castle_adapted(setup_str, strategy, request):
...
def test_create_model_description_castle_adapted_distributed_value_error(mocked_visible_devices, setup_str, request):
    setup = request.getfixturevalue(setup_str)
...
@pytest.mark.parametrize("strategy", ["", "mirrored"])
@pytest.mark.parametrize("setup_str", ["setup_castle_adapted_2d_dagma", "setup_castle_adapted_2d_notears"])
def test_train_and_save_model_description_castle_adapted(setup_str, strategy, request):
    setup = request.getfixturevalue(setup_str)
    setup = set_strategy(setup, strategy)
...
@pytest.mark.parametrize("strategy", ["", "mirrored"])
@pytest.mark.parametrize("setup_str", ["setup_castle_adapted_2d_dagma", "setup_castle_adapted_2d_notears"])
def test_load_model_description_castle_adapted(setup_str, strategy, request):
    setup = request.getfixturevalue(setup_str)
