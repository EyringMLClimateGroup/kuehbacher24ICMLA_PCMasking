...
display_help() {
  ...
  echo ""
  echo "Usage: sbatch -J job_name --output slurm_output_logs --error slurm_error_logs train_castle_split_nodes_batch_jsc.sh -c config.yml -i inputs_list.txt -o outputs_list.txt -x output_index -r percentile -p python_output_dir [-l load_ckp_weight] [-t continue_training] [-s seed] [-j job_name]"
  echo ""
  echo " Options:"
...
# Parse options
while getopts "c:i:o:x:r:s:j:l:t:p:h" opt; do
  case ${opt} in
  h)
    display_help
    exit 0
...
for gpu_index in {0..3}; do
  ...
  conda run --no-capture-output -n kuehbacher1_py3.9_tf python -u main_train_castle_split_nodes_thresholds.py -c "$CONFIG" -i "$INPUTS" -o "$OUTPUTS" -x "$var_index" -r "$PERCENTILE" -l "$LOAD_CKPT" -t "$CONTINUE_TRAINING" -s "$SEED" -g "$gpu_index" >"${PYTHON_DIR}/${JOB_NAME}_python_${var_index}_${SLURM_JOB_ID}.out" 2>&1 &
done
